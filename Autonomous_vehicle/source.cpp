	...
	while (1) {
		...
		if (mode == 2) {
			...
			case 0:
				nowStat = 'R';
				DC_front(); Motor_fast();
				if (rightD > D_D) { Servo_motor(40); }
				else if (rightD > D_C) { Servo_motor(30); }
				else { Servo_motor(15); }
				delay_ms(400);
				break;
			case 1:
				nowStat = 'L';
				DC_front(); Motor_fast();
				if (leftD > D_D) { Servo_motor(-40); }
				else if (leftD > D_C) { Servo_motor(-30); }
				else { Servo_motor(-15); }
				delay_ms(400);
				break;
			...
			case 0:
				nowStat = 'l';
				if (leftD < D_A) { mode = 1; DC_back(); Motor_fast(); Servo_motor(-40); delay_ms(400); Servo_motor(0); delay_ms(300); }
				else if (leftD < D_B) { DC_front(); Motor_fast(); Servo_motor(4 * (D_B - leftD)); }
				break;

			case 1:
				nowStat = 'r';
				if (rightD < D_A) { mode = 1; DC_back(); Motor_fast(); Servo_motor(40); delay_ms(400); Servo_motor(0); delay_ms(300); }
				else if (rightD < D_B) { DC_front(); Motor_fast(); Servo_motor(-4 * (D_B - rightD)); }
				break;
